 ...
 * A `deployment` with a single replica using the `novaadc` container
...
deployeent, understandably, you could be deploying on low resourced kubernetes
...
1. Next to our ADC ("``k8s-solar-system``"), click on the **View Key** button to
  ...
  *It is not necessary to copy the AutoJoin Key for this exercise*, click the
...
1. Now deploy Nova for Kubernetes with our helm chart manifest file we recently
   edited for HTTP service only:
    ...
    change from `not connected` to `initializing`, and then `online` when it is
    ready.
...
1. Inspect the `nova-ns` namespace
    ...
    * the service type `LoadBalancer` has an assigned `EXTERNAL-IP` (This may be
      in a `<pending>` state and take a few minutes to appear)
    
    The `EXTERNAL-IP` address in AWS EKS is a very long Elastic Load Balancer
    (elb) address
   
    Note: On other Cloud providers, such as Digital Ocean, a `EXTERNAL-IP` will
    be a IPv4 Public IP Address may take a few minutes to get assigned

1. Give it some time (<5min) for the `loadBalancer`'s `EXTERNAL-IP` to get
   assigned and become resolvable in DNS. In AWS, this is an elb address, and in
   other cloud providers it could be simply a IP Address.
   
   When the `EXTERNAL-IP` is ready you can test connectivity to your application
   from outside the kuberntes cluster. We will test that in the next exercise.
